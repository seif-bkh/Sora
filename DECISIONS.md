...
| 74 | **The luminance clamp binary-searches the scale factor instead of multiplying and clipping.** | The naive `target/current` scale failed CI on two hostile inputs. (a) A near-black cover needs a factor of ~460, so every channel clipped to 1.0 and the ambient colour came out pure white — the feature died on exactly the dark covers where the glow matters most. (b) Luminance is weighted 0.2126/0.7152/0.0722, so fully saturated blue peaks at 0.0722 and *cannot* reach the 0.26 accent floor by scaling at all; the clamp now desaturates toward white by the smallest amount that reaches the target. ~40 float iterations per cover, cached — nothing next to the Palette pass that produced the input. |
| 75 | **The clamp rounds toward the safe side of whichever bound was crossed.** | sRGB `Color` is 8 bits per channel, so one step is ~0.004 luminance and the search cannot land exactly on a bound. Returning the midpoint of the final interval quantised to just *under* the accent floor and failed CI a second time. Raising to a floor now returns the search's upper endpoint, lowering to a ceiling the lower one, and colours already in band are returned untouched. |
| 76 | **UNVERIFIED locally: no Kotlin in this phase has been compiled.** | The sandbox has no JDK or Android SDK and every Maven/Google host is unreachable. CI is the first compiler to see this code, exactly as in Phases 1a/1b. |
